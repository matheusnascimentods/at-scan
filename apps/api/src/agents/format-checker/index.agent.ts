...
@Injectable()
export class FormatCheckerAgent {
  async checkFormat(resumeContent: string): Promise<FormatCheckerOutputDto> {
    const parsed = await this.run({ resumeContent });

    return FormatCheckerOutputSchema.parse(parsed);
  }
