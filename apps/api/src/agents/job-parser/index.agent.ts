...
export class JobParserAgent {
  async parseJob(jobDescription: string): Promise<JobParserOutputDto> {
    const parsed = await this.run({ jobDescription });

    return JobParserOutputSchema.parse(parsed);
  }
